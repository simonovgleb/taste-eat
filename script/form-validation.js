const authForm = document.querySelector(".auth__form__inputs");
const emailInput = document.querySelector("#email-input");
const emailError = document.querySelector("#email-error");
const passwordError = document.querySelector("#password-error");
const passwordInput = document.querySelector("#password-input");
const confirmInput = document.querySelector("#password-confirmation-input");
const confirmError = document.querySelector("#password-confirmation-error");
const suggestPasswordBtn = document.querySelector("#suggest-password-btn");
...
const fNameInput = document.querySelector("#first-name-input");
const fNameError = document.querySelector("#first-name-error");
const lNameInput = document.querySelector("#last-name-input");
const lNameError = document.querySelector("#last-name-error");
const patronymicInput = document.querySelector("#patronymic-input");
const patronymicError = document.querySelector("#patronymic-error");
const nicknameInput = document.querySelector("#nickname-input");
const nicknameError = document.querySelector("#nickname-error");
const suggestNicknameBtn = document.querySelector("#suggest-nickname-btn");
const tosInput = document.querySelector("#tos-input");
const tosError = document.querySelector("#tos-error");
...
phoneInput.addEventListener("input", () => {
    phoneInput.value = "+375" + phoneInput.value.replace(/\+375/g, "");
    if (!phoneInput.validity.valid) {
        phoneError.style.display = "block";
        if (phoneInput.validity.valueMissing) {
            phoneError.textContent = getTranslation(locale, "auth-empty-phone-number");
        } else if (phoneInput.validity.patternMismatch) {
            phoneError.textContent = getTranslation(locale, "auth-invalid-phone-number");
        }
    } else {
...
phoneInput.addEventListener("keydown", event => {
    if ([8, 46].includes(event.keyCode) && phoneInput.value.length < 5) {
...
dobInput.addEventListener("input", () => {
    if (dobInput.validity.valid) {
        ...
        if (dobInput.validity.valueMissing) {
            dobError.textContent = getTranslation(locale, "auth-empty-dob");
        } else if (dobInput.validity.rangeOverflow) {
            dobError.textContent = getTranslation(locale, "auth-invalid-dob");
        } else if (dobInput.validity.rangeUnderflow) {
            dobError.textContent = getTranslation(locale, "auth-invalid-dob-underflow")
        }
    }
});

passwordInput.addEventListener("input", () => {
    if (compromised.includes(passwordInput.value)) {
    	passwordInput.setCustomValidity("compromised");
	} else {
    ...
    if (passwordInput.validity.valid) {
        ...
        if (passwordInput.validity.valueMissing) {
            ...
            passwordError.textContent = getTranslation(locale, "auth-invalid-password");
        } else if (passwordInput.validity.customError) {
			passwordError.textContent = getTranslation(locale, "auth-compromised-password");
        }
    }
});

confirmInput.addEventListener("paste", event => {
    event.preventDefault();
});
...
confirmInput.addEventListener("input", () => {
	...
	if (confirmInput.validity.valid) {
		...
		if (confirmInput.validity.valueMissing) {
			confirmError.textContent = getTranslation(locale, "auth-empty-confirm");
		} else if (confirmInput.validity.customError) {
			confirmError.textContent = getTranslation(locale, "auth-invalid-confirm");
		}
	}
});
...
fNameInput.addEventListener("input", () => {
    if (fNameInput.validity.valid) {
        ...
        if (fNameInput.validity.valueMissing) {
            fNameError.textContent = getTranslation(locale, "auth-empty-fname");
        } else if (fNameInput.validity.patternMismatch) {
            fNameError.textContent = getTranslation(locale, "auth-invalid-fname");
        } else if (fNameInput.validity.tooShort) {
            fNameError.textContent = getTranslation(locale, "auth-too-short-fname");
        }
	}
});
...
lNameInput.addEventListener("input", () => {
    if (lNameInput.validity.valid) {
        ...
        if (lNameInput.validity.valueMissing) {
            lNameError.textContent = getTranslation(locale, "auth-empty-lname");
        } else if (lNameInput.validity.patternMismatch) {
            lNameError.textContent = getTranslation(locale, "auth-invalid-lname");
        } else if (lNameInput.validity.tooShort) {
            lNameError.textContent = getTranslation(locale, "auth-too-short-lname");
        }
	}
});

patronymicInput.addEventListener("input", () => {
    if (patronymicInput.validity.valid) {
        patronymicError.style.display = "none";
    } else {
        patronymicError.style.display = "block";
        if (patronymicInput.validity.patternMismatch) {
            patronymicError.textContent = getTranslation(locale, "auth-invalid-patronymic");
        } else if (patronymicInput.validity.tooShort) {
            patronymicError.textContent = getTranslation(locale, "auth-too-short-patronymic");
        }
	}
});
...
nicknameInput.addEventListener("input", () => {
    ...
    if (nicknameInput.validity.valid) {
        ...
        if (nicknameInput.validity.valueMissing) {
            nicknameError.textContent = getTranslation(locale, "auth-empty-nickname");
        } else if (nicknameInput.validity.patternMismatch) {
            nicknameError.textContent = getTranslation(locale, "auth-invalid-nickname");
        } else if (nicknameInput.validity.tooShort) {
            nicknameError.textContent = getTranslation(locale, "auth-too-short-nickname");
        } else if (nicknameInput.validity.customError) {
            nicknameError.textContent = getTranslation(locale, "auth-taken-nickname");
        }
	}
});

suggestNicknameBtn.addEventListener("click", () => {
    suggestedNicknames++;
    nicknameInput.value = generateNickname();
    nicknameInput.setCustomValidity("");
    nicknameError.style.display = "none";
...
function generateNickname() {
    let nickname = RANDOM_WORDS_FIRST[getRandomInt(0, RANDOM_WORDS_FIRST.length)] +
        NICKNAME_SEPARATOR +
        RANDOM_WORDS_SECOND[getRandomInt(0, RANDOM_WORDS_SECOND.length)];
...
tosInput.addEventListener("input", () => {
    if (tosInput.validity.valid) {
        tosError.style.display = "none";
    } else {
        tosError.style.display = "block";
        if (tosInput.validity.valueMissing) {
            tosError.textContent = getTranslation(locale, "auth-empty-tos");
        }
	}
});
